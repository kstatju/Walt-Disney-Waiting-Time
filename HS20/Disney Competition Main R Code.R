...
fit  = randomforeset_func(data, prem = c(ntree = 1000, mtry = 44, nodesize = 3), target = "SPOSTMIN1", seed = 12354)
...
write.csv(test_data_pred, file = paste0(datapath, "HS20 TEST DATA PREDICTION.csv", collapse = ''), row.names = FALSE)

# Save fitted Model and Read fitted Model
saveRDS(fit$model, paste0(datapath, "Random_Forest_For_HS20.rds", collapse = ''))
Random_Forest_For_HS20 <- readRDS(paste0(datapath, "Random_Forest_For_HS20.rds", collapse = ''))
