test_data_prep <- function(data = NULL, dataname = NULL, datapath, codepath){
  ...
  tdata["DAYHOURGROUP"] = ifelse(tdata$DAYHOUR >= 9 & tdata$DAYHOUR <= 21, tdata$DAYHOUR, 22)
  ...
  tdata11 <- tdata[ , !(names(tdata) %in% c('SPOSTMIN1', 'SPOSTMIN'))]
  vn = names(tdata11)
  ...
  tdata = cbind(tdata[c('SPOSTMIN1', 'SPOSTMIN')], tdata11)
  
  if(any(!(unlist(varname) %in% names(tdata)))){
    print(unlist(varname)[!(unlist(varname) %in% names(tdata))])
