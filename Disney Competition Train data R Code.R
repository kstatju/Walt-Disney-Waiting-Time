...
train_data_prep <- function(data = NULL, test_data = NULL, dataname = NULL, 
                            test_data_name = NULL, create_test_data = FALSE,
                            test_data_prop = NULL,
                            datapath, codepath, combind = FALSE, seed = NULL){
  datapath = datapath
  codepath = codepath
  if ((!is.null(data) & !is.null(dataname)) |
      (!is.null(test_data) & !is.null(test_data_name))|
      (create_test_data & !is.null(test_data)) |
      (create_test_data & !is.null(test_data_name))){
    message("Either input Train Data or Train Data File name and any one of the test data options 
            test_data, test_data_name and create_test_data")
    return()
  }
  ...
  if(is.null(data) & !is.null(dataname)){
    for (i in 1:length(dataname)){
      data1 =  read.csv(paste0(datapath, dataname[i], ".CSV", collapse = ''), stringsAsFactors = FALSE)
      if (i==1) data = data1
  ...
  season = as.data.frame(sort(table(data$SEASON))/nnrow*100)
  ...
  DLR_TICKET_SEASON = as.data.frame(sort(table(data$DLR_TICKET_SEASON))/nnrow*100)
  ...
  WDW_TICKET_SEASON = as.data.frame(sort(table(data$WDW_TICKET_SEASON))/nnrow*100)
  ...
  PARTYSEASON_WDW = as.data.frame(sort(table(data$PARTYSEASON_WDW))/nnrow*100)
  ...
  if (length(uniqNEWNESS) > 1){
    NEWNESS = as.data.frame(sort(table(data$NEWNESS))/nnrow*100)
  ...
  data['BOOLVAR_DMY'] = rowSums(data[booleanvar])
  ...
  tdata = test_data_prep(data = test_data, dataname = test_data_name, datapath = datapath, codepath = codepath, combind = FALSE)
  
  return(list(Train_Data = data, Test_Data = tdata, data1 = data1))
}