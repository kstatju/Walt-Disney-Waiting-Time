...
randomforeset_func <- function(data, prem, target = "SPOSTMIN1", seed = 12354){
  ...
  for(i in 1:7){
    ...
    if (nrow(data) > 0){
      ...
      train_label  <- as.matrix(as.numeric(as.character(unlist(data[predvar]))))
      
      prem = prem
      classwt = table(train_label)/nrow(data)
      ptm = proc.time()
      result = list()
...
pred_func_rf <- function(test_data, model){
  ...
  for (i in 1:7){
    ...
    if(nrow(test_data) > 0){
      pred = predict(model[[i]], test_data, type="response", na.action = "NA")
...
maeoucom <- function(y, pred = NULL){
  if (dim(y)[2] == 2 & is.null(pred)){
    dif = as.numeric(as.character(y[,1])) - as.numeric(as.character(y[,2]))
  } else dif = as.numeric(as.character(y))-as.numeric(as.character(pred))
