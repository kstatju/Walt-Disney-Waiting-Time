...
train_data_prep <- function(data = NULL, dataname = NULL, 
  ...
  data["DAYHOURGROUP"] = ifelse(data$DAYHOUR <= 10 | data$DAYHOUR == 23, 9, data$DAYHOUR)

  data$DAYHOURGROUP= ifelse(data$DAYHOURGROUP == 11 |
                              data$DAYHOURGROUP == 12, 10, data$DAYHOURGROUP)
  data$DAYHOURGROUP= ifelse(data$DAYHOURGROUP == 13 |
                              data$DAYHOURGROUP == 14, 11, data$DAYHOURGROUP)
  
  data$DAYHOURGROUP= ifelse(data$DAYHOURGROUP == 15 |
                              data$DAYHOURGROUP == 16, 12, data$DAYHOURGROUP)
  data$DAYHOURGROUP= ifelse(data$DAYHOURGROUP == 17 |
                              data$DAYHOURGROUP == 18, 13, data$DAYHOURGROUP)
  data$DAYHOURGROUP= ifelse(data$DAYHOURGROUP == 19 |
                              data$DAYHOURGROUP == 20, 14, data$DAYHOURGROUP)
  data$DAYHOURGROUP= ifelse(data$DAYHOURGROUP == 21 |
                              data$DAYHOURGROUP == 22 , 15, data$DAYHOURGROUP)
 ...
 for(i in 1:length(carvar)){
  ...
  # aa = data[ , !(names(data) %in% c('SPOSTMIN1', 'SPOSTMIN', "DAYHOURGROUP"))]
  # nn = names(aa)
  # for(i in 1:length(nn)){
  #   a = hist(unlist(aa[nn[i]]), plot = FALSE)$breaks
  #   a = c(-Inf, a, Inf)
  #   write.csv(a, file = paste0(datapath, nn[i], "11.csv", collapse = ''), row.names = FALSE)
  #   bb = cut(unlist(aa[nn[i]]), a, labels = FALSE)
  #   aa[nn[i]] = bb
  # }
  # 
  # data = as.data.frame(cbind(data[c('SPOSTMIN1', 'SPOSTMIN', "DAYHOURGROUP")], aa))
